...
describe Generator, '#変換' do
  ...
  context '母音' do
    ...
    it 'シンボルでかな，および，配列で母音の位置を指定して，母音を検査します（あいうえお）' do
      g = Generator.new
      g.変換(:あ行, 母音位置: [{左右: :右, 段: :中, 番号: 0}, {左右: :右, 段: :中, 番号: 1}, {左右: :右, 段: :中, 番号: 2},
  ...
  context '子音' do
    ...
    it '静音を検査します' do
      ...
      g.変換(:さ行, 子音位置: {左右: :右, 段: :中, 番号: 4}, 母音位置: :鍵盤).
        should eq([["sa", "さ"], ["si", "し"], ["su", "す"], ["se", "せ"], ["so", "そ"]])
      g.変換(:た行, 子音位置: {左右: :右, 段: :中, 番号: 2}, 母音位置: :鍵盤).
        should eq([["ta", "た"], ["ti", "ち"], ["tu", "つ"], ["te", "て"], ["to", "と"]])
      g.変換(:な行, 子音位置: {左右: :右, 段: :中, 番号: 3}, 母音位置: :鍵盤).
        should eq([["na", "な"], ["ni", "に"], ["nu", "ぬ"], ["ne", "ね"], ["no", "の"]])
      g.変換(:は行, 子音位置: {左右: :右, 段: :中, 番号: 1}, 母音位置: :鍵盤).
        should eq([["ha", "は"], ["hi", "ひ"], ["hu", "ふ"], ["he", "へ"], ["ho", "ほ"]])
      g.変換(:ま行, 子音位置: {左右: :右, 段: :下, 番号: 1}, 母音位置: :鍵盤).
        should eq([["ma", "ま"], ["mi", "み"], ["mu", "む"], ["me", "め"], ["mo", "も"]])
      g.変換(:や行, 子音位置: {左右: :右, 段: :下, 番号: 3}, 母音位置: :鍵盤).
        should eq([["va", "や"], ["vi", "　"], ["vu", "ゆ"], ["ve", "　"], ["vo", "よ"]])
      g.変換(:ら行, 子音位置: {左右: :右, 段: :上, 番号: 3}, 母音位置: :鍵盤).
        should eq([["ra", "ら"], ["ri", "り"], ["ru", "る"], ["re", "れ"], ["ro", "ろ"]])
      g.変換(:わ行, 子音位置: {左右: :右, 段: :下, 番号: 2}, 母音位置: :鍵盤).
        should eq([["wa", "わ"], ["wi", "ゐ"], ["wu", "　"], ["we", "ゑ"], ["wo", "を"]])
    end

    it '濁音・半濁音を検査します' do
      g = Generator.new
      g.変換(:が行, 子音位置: {左右: :右, 段: :上, 番号: 1}, 母音位置: :鍵盤).
        should eq([["ga", "が"], ["gi", "ぎ"], ["gu", "ぐ"], ["ge", "げ"], ["go", "ご"]])
      g.変換(:ざ行, 子音位置: {左右: :右, 段: :下, 番号: 4}, 母音位置: :鍵盤).
        should eq([["za", "ざ"], ["zi", "じ"], ["zu", "ず"], ["ze", "ぜ"], ["zo", "ぞ"]])
      g.変換(:だ行, 子音位置: {左右: :右, 段: :中, 番号: 0}, 母音位置: :鍵盤).
        should eq([["da", "だ"], ["di", "ぢ"], ["du", "づ"], ["de", "で"], ["do", "ど"]])
      g.変換(:ば行, 子音位置: {左右: :右, 段: :下, 番号: 0}, 母音位置: :鍵盤).
        should eq([["ba", "ば"], ["bi", "び"], ["bu", "ぶ"], ["be", "べ"], ["bo", "ぼ"]])
      g.変換(:ぱ行, 子音位置: {左右: :右, 段: :上, 番号: 0}, 母音位置: :鍵盤).
  ...
  context '撥音化' do
    ...
    it '子音+母音+撥音（一括）を検査します' do
      g = Generator.new
      g.変換("かきくけこ", 子音位置: {左右: :右, 段: :上, 番号: 2}, 撥音化: {}, 母音位置: {左右: :左, 段: :下}).
  ...
  context '拗音化' do
    it '子音＋拗音化＋母音を検査します' do
      g = Generator.new
      g.変換(:きゃ行, 子音位置: {左右: :右, 段: :上, 番号: 2}, 拗音化: {左右: :右, 番号: 1}, 母音位置: :鍵盤).
