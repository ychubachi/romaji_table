...
describe Generator do
  ...
  describe '#変換' do
    ...
    it '文字に追加する文字を渡すことで，撥音や促音などを生成できる' do
      ...
      r = s.変換(:か行, 追加: 'ん', 開始鍵: {左右: :右, 段: :上, 番号: 2}, 確定鍵: {左右: :左, 段: :下})
    ...
    it '制限事項' do
      ...
      }.to raise_error 'かなは確定鍵と同じ文字数で指定してください'
      expect{
        s.変換(:あ行, 確定鍵: [{左右: :右, 段: :中, 番号: 0}])
      }.to raise_error 'かなは確定鍵と同じ文字数で指定してください'
      expect{
        s.変換(nil, 確定鍵: [{左右: :右, 段: :中, 番号: 0}])
      }.to raise_error 'かなはシンボル，文字列，配列で指定してください'
      expect{
        s.変換(:あ行, 確定鍵: :母音)
      }.to raise_error '確定鍵位置は配列，連想配列またはnilで指定してください'
      expect{
