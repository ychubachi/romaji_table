...
describe Generator do
  ...
  describe '#変換' do
    it '行と，確定鍵の位置の配列を与えると，対応表を返す（あいうえお）' do
      r = s.変換(:あ行, 確定鍵: [{左右: :右, 段: :中, 番号: 0}, {左右: :右, 段: :中, 番号: 1},
                                 {左右: :右, 段: :中, 番号: 2}, {左右: :右, 段: :中, 番号: 3},
                                 {左右: :右, 段: :中, 番号: 4}])
      expect(r).to eq([["d", "あ"], ["h", "い"], ["t", "う"], ["n", "え"], ["s", "お"]])
      expect{
        s.変換(:あ行, 母音: [{左右: :右, 段: :中, 番号: 0}])
      }.to raise_error
    end

    it '行を与えて，確定鍵を省略すると，対応表を返す' do
      s.変換(:あ行).
        should eq([["a", "あ"], ["i", "い"], ["u", "う"], ["e", "え"], ["o", "お"]])
    end

    it '文字に追加する文字を渡すことで，撥音や促音などを生成できる' do
      r = s.変換("あいうえお", 追加: 'ん', 確定鍵: {左右: :左, 段: :下})
      expect(r).to eq([[";", "あん"], ["x", "いん"], ["k", "うん"], ["j", "えん"], ["q", "おん"]])
      r = s.変換("かきくけこ", 追加: 'ん', 開始鍵: {左右: :右, 段: :上, 番号: 2}, 確定鍵: {左右: :左, 段: :下})
    ...
    it '文字列でかな，および，鍵盤の位置を指定して母音を検査します' do
      r = s.変換('あいうえお')
      expect(r).to eq([["a", "あ"], ["i", "い"], ["u", "う"], ["e", "え"], ["o", "お"]])
      expect{
        r.変換('いうえお', 確定鍵: {左右: :左, 段: :中})
      }.to raise_error
    end

    it '文字列でかな，および，母音の位置を指定して，母音を検査します（あいうえお）' do
      g = Generator.new
      g.変換('あいうえお', 確定鍵: {左右: :左, 段: :中}).
        should eq([["a", "あ"], ["i", "い"], ["u", "う"], ["e", "え"], ["o", "お"]])
    end
    ...
    it '文字に直音の行，位置を省略して呼ぶと，対応表を返します' do
  ...
  describe '#省略R' do
    it '鍵の位置を省略して指定します' do
      ...
      expect{s.send(:省略R, 位置: {左右: :左, 番号: 0})}.to raise_error
