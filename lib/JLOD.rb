...
[[:きゃ行, :か行], [:しゃ行, :さ行], [:ちゃ行, :た行], [:にゃ行, :な行],
 [:ひゃ行, :は行], [:みゃ行, :ま行], [:りゃ行, :ら行],
 [:ぎゃ行, :が行], [:じゃ行, :ざ行], [:ぢゃ行, :だ行],
 [:ばゃ行, :ば行], [:ぴゃ行, :ぱ行],
 [:ゃ行,   :や行],
].each do |行, 鍵|
  if 鍵
    拗音鍵 = 子音[鍵][:番号] < 2 ? 薬指 : 人差指
...
2) 通常のローマ字綴りでは[H]キーを用いる表記は，JLODでは，子音キーと同じ段(同じ並び)の右手人差し指か薬指のどちらか打ちにくい指（い行＋ゃぃゅぇょの場合とは異なる指），で入力します．[H]キーは使えません．
...
[[:ぴゃ行, :ぱ行], [:ぎゃ行, :が行], [:きゃ行, :か行], [:りゃ行, :ら行],
 [:ぢゃ行, :だ行], [:ひゃ行, :は行], [:ちゃ行, :た行], [:にゃ行, :な行], [:しゃ行, :さ行],
 [:びゃ行, :ば行], [:みゃ行, :ま行], [:じゃ行, :ざ行]].each do |行, 鍵|
  変換 二重母音(行), 子音: 子音[鍵], 母音: [拗音母音L, 拗音母音R]
end
...
中央段に関しては，母音＋「つ」が入力できます．
下段に関しては，母音＋「っ」が入力できます．
上段に関しては，え段は母音＋「き」，それ以外は母音＋「く」が入力できます．
あ行，ふぁ行，う゛ぁ行に関してはこの省略打ちは出来ません．
...
よく使う語句を簡単に入力できるようにしています．この部分の語句は人により頻度が違うと思いますので，自分用にカスタマイズした方がよいと思います． 他の基本ルールと混ざらないよう，同段のキー２つには割り当てないようにしています．
