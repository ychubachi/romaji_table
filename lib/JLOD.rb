...
変換 'っ', 子音: L, 拗音化: 子音[:た行], 母音: う # ltu -> 辞書になし
...
=begin
拗音行.each do |行|
  拗音鍵 = 子音[行][:番号] < 2 ? 薬指 : 人差指
  変換 拗音(行, :い列, :ゃ行),           子音: 子音[行], 拗音化: 拗音鍵,             母音: 左中 # きゃ
  変換 拗音(行, :い列, :ゃ行),           子音: 子音[行], 拗音化: 拗音鍵, 撥音化: {}, 母音: 左下 # きゃん
  変換 二重母音(拗音(行, :い列, :ゃ行)), 子音: 子音[行], 拗音化: 拗音鍵,             母音: 左上 # きゃい，きゅい，…
end
=end
...
# [:は行, :わ行].each do |行|
#   拗音鍵 = 子音[行][:番号] < 2 ? 人差指 : 薬指
#   拗音 = 拗音(行, :う列, :ぁ行)
#   変換 拗音,           子音: 子音[行], 拗音化: 拗音鍵,             母音: 左中
#   変換 拗音,           子音: 子音[行], 拗音化: 拗音鍵, 撥音化: {}, 母音: 左下 # きゃん
#   変換 二重母音(拗音), 子音: 子音[行], 拗音化: 拗音鍵,             母音: 左上 # きゃいきゅいきゅうきぇいきょう
# end
...
# 拗音行.each do |行|
#   変換 二重母音(拗音(行, :い列, :ゃ行)), 子音: 子音[行], 母音: [拗音母音L, 拗音母音R]
# end
...
#   拗音鍵 = 子音[行][:番号] < 2 ? 薬指 : 人差指
#   拗音 = 拗音(行, :い列, :ゃ行)
#   変換 二重母音(拗音), 開始鍵: 子音[行], 中間鍵: 拗音鍵, 確定鍵: [人差指, 薬指, 小指, 中指]
# end
...
# H-3
あ行以外.each do |行|
  変換 行, 追加: 'つ', 開始鍵: 子音[行], 中間鍵: 人差指左, 確定鍵: 左中
end

# H-4
あ行以外.each do |行|
  変換 行, 追加: 'っ', 開始鍵: 子音[行], 中間鍵: 人差指左, 確定鍵: 左下
end
...
あ行以外.each do |行|
  変換 二重母音(行), 開始鍵: 子音[行], 中間鍵: 人差指左, 確定鍵: 左上
end
