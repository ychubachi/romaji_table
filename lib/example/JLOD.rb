...
def rule_c
  @拗音行.each do |行|
    ...
    変換 拗音行(行, :い列, :ゃ行),                 開始鍵: @開始鍵[行], 中間鍵: 拗音鍵, 確定鍵: @左中 # きゃ
    変換 拗音行(行, :い列, :ゃ行), 追加文字: 'ん', 開始鍵: @開始鍵[行], 中間鍵: 拗音鍵, 確定鍵: @左下 # きゃん
    変換 二重母音(拗音行(行, :い列, :ゃ行)),       開始鍵: @開始鍵[行], 中間鍵: 拗音鍵, 確定鍵: @左上 # きゃい，きゅい，…
  end
end
...
def rule_f
  ...
  @拗音行.each do |行|
    変換 二重母音(拗音行(行, :い列, :ゃ行)), 開始鍵: @開始鍵[行], 確定鍵: [拗音母音L, 拗音母音R]
  end
end
...
def rule_h
  ...
  # 拗音を追加
  @拗音行.each do |行|
    変換 拗音行(行, :い列, :ゃ行), 追加文字: 'っ', 開始鍵: @開始鍵[行], 中間鍵: @人差指左, 確定鍵: @左上
  end
